...
The `stat_function()` produces a normal curve using the function `dnorm()`. The rest of the command specified the mean as being the mean of the numerical_var after removing any missing values and the standard deviation as being that of numerical_var.
...
## Multicollinearity (todo)

WILL COME BACK TO THIS IN THE REGRESSION CHAPTER (page 292). Only if you have multiple predictors (like in multiple regression or factorial ANOVA). For simple t-tests or one-way ANOVA with one factor, multicollinearity isn’t an issue.

Multicollinearity affects the stability and interpretability of the model coefficients. You need valid predictors first before assessing residual behavior like homoscedasticity. Thus test before homogeneity/homoscedasticity.
...
### Testing homoscedasticity (todo)

WILL COME BACK TO THIS IN THE REGRESSION CHAPTER (page 294).

In correlational analysis such as regression we tend to use graphs for testing homoscedasticity.
...
#### Outlier detection in categorical data (todo)

Categorical outlier detection can be done with a single variable (e.g., rare categories) or across multiple variables using a multivariate

**Univariate**

Based on low frequency or proportion in the data—typically categories with counts or percentages below a chosen threshold (e.g., less than 1% of total observations).

How is a threshold decided? - Common practice uses fixed cutoffs (e.g., 1% or 5%) for rarity. - Domain knowledge can guide stricter or looser thresholds. - Statistical tests or modeling (e.g., chi-square, clustering) can suggest unusual frequencies.

However, rare cases can indicate unbalanced sample rather than outliers which could further indicate a faulty study design.

**Multivariate** - Look into gower's distance (cluster::daisy() in R) - Encode categorical variables carefully (e.g., dummy codes) if included.

## Sensitivity Analyses (todo)

When we are not sure about how the methods used during data preparation affect our data, we can use sensitivity analyses. These simply mean generating a set of results for each method used. For example, we can perform the statistical analysis while leaving outliers in, and then comparing to another set of results where outliers have been removed.


## Transformations (todo)

Sometimes transformations improve the distribution of the data.
