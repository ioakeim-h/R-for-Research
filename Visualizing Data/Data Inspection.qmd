...
When `everything()` is used, the resulting dataset has only two columns:
- A categorical column (Variable) containing the original column names.
- A numeric column (Value) containing the original values.
...
`myDataLong$myFactor <- as.factor(myDataLong$myVariable)`

We can rename the levels of a factor so that they appear as desired on the plot:
`levels(myDataLong$myFactor) <- c("renamedLevel1","renamedLevel2")`
...
`group = 1` is used because we are joining summary points, otherwise the line will not display.
`fun.data = mean_cl_boot` because the error bars on graphs of repeated-measures designs are not corrected for the fact that the data points are dependent (read section 9.2 in Discovering Statistics Using R).
